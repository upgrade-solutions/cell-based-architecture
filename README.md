...
The `node/express` and `node/nestjs` adapters expose Swagger UI (`/api`), Redoc (`/docs`), and raw OpenAPI JSON (`/api-json`). The `node/fastify` adapter is **Redoc-only** — `/` redirects to `/docs`, plus `/api-json` for the raw spec — because Redoc's built-in "Request samples" panel already provides copy-curl, and dropping `@fastify/swagger` + `@fastify/swagger-ui` removes the v9/v5 plugin-API fragility that motivated the `fix-fastify-adapter-swagger-ui-wiring` change.

### `compute` hint (ECS vs Lambda)

The Fastify adapter accepts a `compute: 'ecs' | 'lambda'` cell config. The default is `ecs` — the same long-running server every other Node adapter produces. Set `compute: 'lambda'` to also emit a Lambda-targeted entrypoint that wraps Fastify with `@fastify/aws-lambda@^5` in streaming mode (`awslambda.streamifyResponse`), suitable for Lambda Function URLs with `invoke_mode = RESPONSE_STREAM`. The generated `package.json` pins fastify@^5 plus a v5-aligned plugin set (`@fastify/cors@^11`, and `@fastify/aws-lambda@^5` only when `compute: 'lambda'`); these versions move in lockstep — drift in any one re-introduces `FST_ERR_PLUGIN_VERSION_MISMATCH` at startup. If you see that error, regenerate the cell against the current cba (older generated trees pinned a mismatched set). The adapter deliberately does **not** carry `@fastify/swagger` or `@fastify/swagger-ui`: Redoc renders the OpenAPI doc at `/docs` directly from the cell-built spec, so the plugin pair was dead weight and a recurring source of v9/v5 wiring bugs.
