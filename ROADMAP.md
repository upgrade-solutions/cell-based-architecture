...
- [x] Define `Signal` schema (`@dna-codes/dna-schemas/operational/signal.json`) — name, capability, description, typed payload fields
- [x] Add `signals` array to operational composite schema (`@dna-codes/dna-schemas/operational/operational.json`)
...
- [x] Vendor mode — primitives at `@dna-codes/cells-ui/primitives` by default (vendored into output); set `vendorComponents: false` in adapter config to import from the workspace package instead
...
1. `@dna-codes/dna-schemas/product/web/layout.json` — add `"universal"` to the type enum, add optional `features` and `tenants` properties
...
- [x] **`@dna-codes/dna-schemas/product/product.core.json`** — new schema for the self-contained product-core document. Flat `nouns[]` at the top level plus optional `capabilities`, `causes`, `rules`, `outcomes`, `lifecycles`, `signals`, `relationships`, `equations`. Registered in the dna-validator.
...
- **Schemas are the single source of truth** for form generation. `@dna-codes/dna-schemas/operational/*.json`, `@dna-codes/dna-schemas/product/*.json`, and `@dna-codes/dna-schemas/technical/*.json` already describe every primitive; RJSF (`@rjsf/core`) renders them into the inspector panel without hand-written editor code.
...
- [x] **Operational types + loader** — `packages/cba-viz/src/loaders/operational-loader.ts` mirrors `@dna-codes/dna-schemas/operational/*.json` in TypeScript interfaces. `parseOperationalDNA`, `loadOperationalDNA` functions.
