 ...
 * endpoint satellites. Mirrors OperationalCanvas's paper setup, zoom,
 * pan, and fade-in behaviors so every layer has the same interaction
 * baseline. When we have four or more canvas components it'll be
 * worth extracting a `useJointPaper` hook — for now the duplication
 * is controlled and each canvas stays independently readable.
 */
export const ProductApiCanvas = observer(function ProductApiCanvas({ model, dna }: ProductApiCanvasProps) {
  ...
  useEffect(() => {
    ...
    const cells = productApiToGraphCells(dna)
